...
- Minor copyedit fixes (compression pass): a grammatical "A envelope" → "An envelope" in the Real-World Story, and an ambiguous "the server" (client/VPN-server/destination-server all appear in the same paragraph) clarified to "the VPN server" in the Technical Explanation.
