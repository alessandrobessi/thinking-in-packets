...
**9. Establishing TLS.** Inside that TCP connection, the browser and server negotiate encryption, the server presents a certificate the browser validates against a trusted certificate authority and the requested hostname, and both sides derive a shared secret via key exchange (Ch. 18). From this point on, the HTTP data carried inside this TLS session is encrypted and integrity-protected — the surrounding IP and TCP headers routing it there, and metadata like packet sizes and timing, remain visible to anyone positioned to observe the path, exactly as Chapter 18 described.
...
There is no new mechanism to explain here — that is the point of this chapter. What's worth being explicit about is the *shape* of the dependency chain for the conventional, first-visit TCP-and-TLS path this chapter follows: local configuration (step 2) before anything else; DNS resolution (steps 3-7) before transport state, since transport needs an address, not a hostname; transport state (step 8) before TLS (step 9), since TLS runs on top of an established TCP connection; and, on a genuinely fresh connection, TLS complete before HTTP (step 10) sends anything, since the request is itself protected data inside that session. That ordering isn't a universal law, though — a returning client can resume a session and send early data before its handshake finishes, and Chapter 24 shows a transport that combines the transport and TLS steps this chapter keeps sequential — points the Common Misconceptions and What to Remember return to.
...
**Correct intuition:** For this chapter's conventional TCP-and-TLS path, the dependency ordering holds, but steps are conditionally skipped when cached state already satisfies them, step 10 repeats many times in parallel for a page's various resources, and — as Chapter 24 shows — not every transport even keeps transport and TLS as two separate sequential steps.
...
- On a genuinely fresh connection, TLS must complete before HTTP (Ch. 19) sends anything — but a returning client can resume a session and send early data first, over plain TCP, independent of QUIC (Chapter 24).
...
- This chain's order describes the conventional TCP-and-TLS path; how much of it runs on a given request depends on what's already cached, leased, or open, and Chapter 24 shows a design that combines steps this chapter keeps sequential.
