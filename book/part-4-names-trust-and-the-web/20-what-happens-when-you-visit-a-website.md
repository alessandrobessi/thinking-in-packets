...
**5. Sending the DNS query.** With the gateway's link-layer address resolved, the laptop can now actually reach its DNS resolver, itself possibly several hops away. The query travels as a UDP datagram (Ch. 13) — DNS doesn't need TCP's ordered byte stream for a single small question-and-answer exchange.
