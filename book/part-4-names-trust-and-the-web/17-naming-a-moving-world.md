...
**Second lookup, a minute later.** This time, the recursive resolver doesn't repeat any of that. It checks its own cache first, finds the answer it stored a minute ago, confirms the TTL hasn't expired yet, and returns it immediately — no root server, no `.net` server, no authoritative server involved at all. Once that specific answer's TTL does expire, the resolver doesn't necessarily repeat the *entire* walk from the root, either: it also separately cached which servers are authoritative for `example.net` along the way, usually with their own, often longer, TTL, so it can often go straight back to asking that same authoritative server again rather than re-discovering it through `.net`'s servers and the root. Only once *that* cached delegation information has also expired — a comparatively rare event — does a resolver actually have to re-walk the full chain from the root. And any resolver with a genuinely empty cache, encountering the name for the first time, always starts from the root regardless.
...
- Every record's TTL bounds how long a resolver may cache it before re-checking with the authoritative source — and a resolver caches delegation information (which servers are authoritative) separately from final answers, often with a longer TTL, so a resolver doesn't necessarily repeat the full root-to-authoritative walk just because one answer expired.
