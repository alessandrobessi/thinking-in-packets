...
**Tunnels and VPNs.** A tunnel carries one packet, header and all, encapsulated inside the payload of another packet, so it can cross a network that wouldn't otherwise carry it directly — including, potentially, carrying a private address across the public Internet by wrapping it inside a publicly addressed outer packet. The consumer and remote-access VPNs most readers actually use combine a tunnel with encryption: the client wraps the original traffic, encrypts it, and sends it to a VPN server, which decrypts it and forwards the original traffic onward as if it originated there. Every device between the client and the VPN server sees only encrypted tunnel traffic to the VPN server's address — they cannot see the original destination or content. But the VPN server itself sees the decrypted, unwrapped traffic and its real destination, since it has to, in order to forward it. A VPN shifts who can observe your traffic; it does not eliminate the possibility of observation. Not every technology marketed as a VPN adds encryption, either — some provider-operated VPN technologies exist purely to keep one customer's traffic logically separated from another's across shared infrastructure, with confidentiality left to whatever runs on top; "VPN" names a tunneling-plus-separation pattern first, and encryption is its most common but not universal companion.
