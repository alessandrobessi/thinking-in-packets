...
**Tunnels and VPNs.** A tunnel carries one packet, header and all, inside the payload of another, so it can cross a network that wouldn't otherwise carry it — including a private address crossing the public Internet inside a publicly-addressed outer packet. The consumer and remote-access VPNs most readers actually use combine a tunnel with encryption: the client encrypts the original traffic and sends it to a VPN server, which decrypts it and forwards it onward as if it originated there. Devices between the client and the server see only encrypted tunnel traffic; what the server itself can and can't see — and why "encrypted" doesn't mean "anonymous" — is the subject of this chapter's VPN misconception below. Not every technology marketed as a VPN even adds encryption: some provider-operated VPNs exist purely to keep one customer's traffic logically separated from another's, leaving confidentiality to whatever runs on top — "VPN" names a tunneling-plus-separation pattern first, and encryption is its most common, not universal, companion.
