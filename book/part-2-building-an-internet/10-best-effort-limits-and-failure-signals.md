...
None of these three packets vanished for no reason — a broken route, an oversized packet, and a full queue are each a real, explainable limit. But only two of them are designed to produce a signal back to the sender at all, and even that designed signal isn't a guarantee — it still has to actually be generated and actually arrive, a point the Technical Explanation below returns to. The third case, arguably the most common in practice, was never designed to produce one in the first place. That asymmetry matters: best-effort delivery means exactly what it says — a genuine attempt, not a promise to always explain a failure when one happens.
...
**ICMP** (Internet Control Message Protocol, with a parallel ICMPv6 for IPv6) is the mechanism IP uses to report certain kinds of failure back to a packet's original sender — messages like "Time Exceeded" (TTL/Hop Limit reached zero), "Destination Unreachable" (no route exists, or the destination refuses the packet), and "Fragmentation Needed" / "Packet Too Big" (the MTU problem below). ICMP is carried directly over IP, the same way any other packet is, and its purpose is turning failures that would otherwise be silent into something the sender can observe and, if the application above it cares to, act on — but ICMP is not generated for every kind of loss, and even when it is generated, it isn't guaranteed to arrive. A router facing a full outbound queue drops packets without sending anything back at all; that's simply not one of the cases ICMP was built to report. And a router or firewall along the path may deliberately rate-limit or filter ICMP traffic — a common operational practice, since ICMP has also been used for reconnaissance and denial-of-service traffic — which can suppress even the messages that would otherwise have been generated. ICMP narrows the range of genuinely silent failures; it does not eliminate silent failure as a possibility.

**MTU** is the largest IP packet, including its IP header, that a given link can carry without IP-layer fragmentation — a physical or configured limit of that specific link, not a property of IP itself. It's a statement about the IP packet, not the link-layer frame that ends up carrying it: the frame is actually somewhat larger than the MTU figure, since it adds its own link-layer header (and sometimes trailer) on top of the IP packet it's carrying — an Ethernet link with a 1500-byte MTU, for instance, carries an IP packet up to 1500 bytes inside a frame that's itself a bit bigger than that once Ethernet's own header is added. A path from source to destination typically crosses several links, potentially with different MTUs, and the smallest MTU anywhere along that path constrains how large a packet can travel the whole way without running into trouble — informally, the **Path MTU**. **Fragmentation** is IPv4's in-transit solution when a packet exceeds the MTU of a link it needs to cross: a router splits it into smaller pieces, each with enough header information to be reassembled at the final destination. IPv6 deliberately removed in-transit fragmentation from routers entirely: a router facing an oversized IPv6 packet simply drops it and sends back a "Packet Too Big" message rather than fragmenting it itself. The sending endpoint remains free to fragment its own packets, using IPv6's Fragment extension header, but in practice usually avoids needing to by choosing an appropriately small packet size up front, informed by Path MTU Discovery's ICMPv6 feedback — pushing the size-limiting work to the edge of the network rather than every router along the path.
...
Together, TTL/Hop Limit, ICMP, and MTU handling are what keep best-effort delivery from meaning "unaccountable" delivery: packets that can't continue are bounded in how much damage they can do, and for specific, well-defined failure types, someone finds out. That's narrower than "someone always finds out" — the most common cause of ordinary packet loss, a congested queue with nowhere to put an arriving packet, produces no signal at all, which is exactly why the transport layer (starting Chapter 13) can't simply assume IP will tell it when something went wrong.
