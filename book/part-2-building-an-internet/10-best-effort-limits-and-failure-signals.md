...
None of these three packets vanished for no reason — a broken route, an oversized frame, and a full queue are each a real, explainable limit. But only two of them reliably produce a signal back to the sender. The third, arguably the most common in practice, does not, and that asymmetry matters: best-effort delivery means exactly what it says — a genuine attempt, not a promise to always explain a failure when one happens.
