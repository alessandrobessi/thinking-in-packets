...
**Under a QUIC-based design.** QUIC identifies the connection by a **connection ID** it chooses itself, independent of IP address or port — so when the phone's address changes, QUIC can keep the same connection going under the new path, a capability called **connection migration**, instead of tearing it down and rebuilding. And because video, audio, and chat ride on genuinely **independent streams** at the transport layer (not layered over one shared byte stream the way Chapter 23's HTTP/2 streams were), a lost packet carrying only audio holds up just the audio stream; video and chat aren't stuck behind it the way TCP's single ordered byte stream makes everything wait on any loss. The Technical Explanation adds two honest qualifications to that picture, but the core win is real: one stream's loss no longer blocks the others.
...
Within one QUIC connection, multiple **independent streams** run concurrently, each with its own delivery order — the structural difference from HTTP/2-over-TCP (Chapter 23), where streams were multiplexed over one shared ordered byte stream. So a stream with no data in a lost packet is never blocked behind another stream's missing bytes, the way TCP forces every stream to wait on any loss. Two qualifications keep this honest. First, a single QUIC packet can bundle frames from several streams for efficiency, so losing it can delay every stream whose data rode inside — QUIC doesn't guarantee a loss touches only one stream. Second, loss detection and congestion control operate on the connection as a whole, not per stream: a loss shrinks the connection's shared congestion window (Chapter 15's logic), briefly slowing every stream's sending rate, even ones that lost nothing of their own. What QUIC removes is one stream's loss *blocking* delivery on unrelated streams — not every effect of it reaching them.

QUIC also integrates encryption (Chapter 18's TLS 1.3) directly into its handshake and treats most of its own transport-level control information — not just the application payload — as **encrypted transport metadata**, hiding far more from network intermediaries than TCP's unencrypted header ever did. One consequence: because setup and cryptographic handshake are combined, a QUIC connection to a recently-seen server can, under the right conditions, send application data alongside its very first handshake message — **zero-round-trip (0-RTT) resumption**. That early-data capability comes from TLS 1.3 itself, not QUIC — a plain TCP-then-TLS connection can do it too — but QUIC additionally removes the separate round trip a fresh TCP-then-TLS connection needed just to establish the transport connection before TLS could begin.
