...
**Under a QUIC-based design.** The connection is identified by a **connection ID** chosen by QUIC itself, independent of IP address or port — so when the phone's IP address changes from switching networks, QUIC can recognize the same ongoing connection continuing under a new network path and keep going, a capability called **connection migration**, without tearing anything down or renegotiating from scratch. And because QUIC's video, audio, and chat data ride on genuinely **independent streams** at the transport level itself (not just at the HTTP level, the way Chapter 23's HTTP/2 streams still shared one underlying TCP byte stream), losing a packet that happened to carry only audio data stalls just the audio stream's own delivery, with video and chat continuing normally in the meantime. If that same lost packet had also been carrying a video or chat frame bundled in alongside the audio data — QUIC packs frames from multiple streams together when it can — those streams would wait too, for their own missing piece; what QUIC actually guarantees is narrower but still the meaningful part: a stream with none of its own data in the lost packet is never forced to wait behind someone else's problem, unlike TCP's single shared byte stream, which stalls everything on any loss regardless of which logical stream the lost bytes belonged to.
...
Within one QUIC connection, multiple **independent streams** can be open concurrently, each with its own delivery order — this is the crucial structural difference from HTTP/2-over-TCP (Chapter 23): QUIC's streams are independent at the *transport* layer itself, not layered as a multiplexing trick on top of one shared ordered byte stream. Loss detection, acknowledgement, and congestion response happen at the level of the connection as a whole, tracking QUIC packets rather than keeping fully separate bookkeeping per stream — but when a packet is lost, only the specific stream data that packet was carrying needs to be resent, as a new frame, and only that stream's own delivery waits on it; a stream with none of its data in the lost packet is simply never touched. A single QUIC packet on the wire can still carry frames belonging to more than one stream bundled together for efficiency, so losing that one packet can delay every stream whose data happened to ride inside it — QUIC doesn't guarantee that a lost packet only ever touches one stream. What it does guarantee is narrower but still the important part: a stream with no data in the lost packet is never forced to stall behind another stream's retransmission the way TCP's one shared ordered byte stream would force it to. The benefit is the removal of *unrelated*-stream blocking, not a promise that loss is always perfectly contained to a single stream, and not a claim that every stream keeps fully independent loss-tracking machinery of its own.
...
sequenceDiagram
    ...
    S-->>P: Video and chat streams, untouched by that packet, keep delivering normally
```

*Alt text: A phone maintains one QUIC connection, identified by a connection ID rather than its IP address, across a switch from Wi-Fi to cellular; losing a packet that carried only audio data delays just the audio stream, while video and chat streams — whose data was not in that packet — keep delivering independently. A packet carrying more than one stream's data would delay each of those streams.*
...
- QUIC's streams have independent delivery order, so a stream with no data in a lost packet never stalls behind another stream's retransmission — though loss detection and congestion response happen at the connection level, and one lost QUIC packet can still carry frames from more than one stream at once.
