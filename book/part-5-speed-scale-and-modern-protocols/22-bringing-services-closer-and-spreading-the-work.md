...
A **load balancer** sits in front of a pool of interchangeable backend servers and decides which one handles incoming work, spreading demand so none is overwhelmed while others sit idle. *How often* it decides is a design choice: an application-layer balancer can pick fresh per HTTP request, while a transport-layer one more commonly picks once per TCP connection or UDP flow, and every subsequent request on that connection rides along with it. A **health check** is how the load balancer knows which backends can actually take traffic: a periodic probe that pulls a backend from rotation, without a human noticing first, whenever that probe judges it unfit — though only as reliably as what the probe actually checks, a limit the Common Misconceptions return to.

**Replication** means running multiple copies of a service or its data — what load balancing across *equivalent* backends depends on, since there's nothing to balance across otherwise (a balancer can also split heterogeneous or partitioned backends, which is a distribution problem rather than a replication one). Replication buys capacity and resilience, but raises a question: if a user's session lives only in one replica's memory, routing their next request elsewhere means that replica doesn't have it. **Affinity** (session stickiness) routes a user's repeated requests back to the same backend to sidestep exactly that — at the cost of that backend no longer being freely swappable mid-session. What affinity does and doesn't fix is a common enough confusion to get its own misconception below.
