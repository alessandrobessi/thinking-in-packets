...
Every UDP datagram and every TCP segment — the two transport protocols this book covers, introduced formally in Chapters 13-14 — includes both a **source port** and a **destination port** — the port the sending process used, and the port the receiving process is expected to be listening on. (Not every protocol carried by IP uses ports: ICMP, Chapter 10's failure-signaling mechanism, operates alongside IP as a control protocol and carries no port numbers at all. UDP and TCP do use them — that's the pattern this chapter needs, without claiming it as a universal property of everything above IP.) Combined with the source and destination IP addresses, and the transport protocol in use (Ch. 13-14 introduce the two main options, UDP and TCP), these five values together form what's conventionally called a **five-tuple**: (source IP, source port, destination IP, destination port, protocol). Within a given network context, at a given moment, this five-tuple identifies one transport flow — and, for TCP specifically, one established connection — not the IP address alone, and not the port alone, but the specific combination of all five together. That scoping matters for a real reason, not just as a hedge: TCP's own handshake state makes "one five-tuple, one connection" hold precisely, but UDP has no connection to establish in the first place, so the same five values can legitimately carry more than one independent exchange even without anything having "ended" first — and, separately, once a TCP connection genuinely does end, the same five values can eventually be reused for an entirely new, unrelated one.

This is what makes it possible for a server to hold open connections with thousands of different clients on the very same listening port at once. Every one of those connections shares the same destination IP and destination port on the server side, but each has its own distinct combination of source IP and source port, so every five-tuple is still unique — even behind NAT (Chapter 16), where many different clients can share one public source IP, their source ports still keep each connection distinct. The server isn't tracking "the connection on port 443" as if there could only be one; it's tracking many separate tuples that all happen to share that one destination port.
...
**Demultiplexing** is the general name for this matching process: given an incoming packet, using its five-tuple to determine which one process, out of everything running on the machine, should actually receive it. A listening socket hasn't been matched to a full five-tuple yet — no remote endpoint exists until someone connects — so demultiplexing to *it* specifically works off whatever it's actually bound to: at minimum the destination port and transport protocol together (a TCP listener and a UDP listener can share the same port number without conflict, since they differ in protocol), and, if the socket was bound to one specific local address rather than all of them, that local address too. It is the mirror image of what the operating system did when the outgoing packets from three different processes were, in the reverse direction, all sent out sharing the same source IP address.
